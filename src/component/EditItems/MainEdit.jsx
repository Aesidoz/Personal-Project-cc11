...
function MainEdit() {
  ...
  useEffect(() => {
    const fetchById = async (id) => {
      try {
        const result = await axios.get(`/posts/public/${id}`);
        setPostId(result.data.posts);
        setTitle(result.data.posts.title);
        setDiscription(result.data.posts.discription);
        setCountry(result.data.posts.country);
        setProVince(result.data.posts.proVince);
        setBudGet(result.data.posts.budGet);
        setContent(result.data.posts.mainContent);
  ...
  const postReview = async (e) => {
    ...
    try {
      ...
      const result = await axios.patch(`/posts/${id}`, reviewData);
    } catch (err) {
      console.log(err);
    } finally {
      navigate("/");
  ...
  return (
    <div className="fs-1 bg-secondary">
      <div className="text-center brown ">EDIT CONTENT</div>
